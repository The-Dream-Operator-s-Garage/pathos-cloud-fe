...
.label-dock {
  ...
  // ⚠ THE FAMILY MOVED ON 2026-09-07 (user ask: "labels -> red") — the levels named here
  // are red's now; the dials above them follow in `_tokens.scss`.
  --dock-rule-strong: var(--red-9);
...
.label-dock__chain-link {
  ...
  &:hover { color: var(--labels-contrast); background: rgba(var(--red-7-rgb), 0.1); }
...
.ltm-badge {
  ...
  &--mine { color: var(--labels-contrast); background: rgba(var(--red-7-rgb), 0.12); }
}
