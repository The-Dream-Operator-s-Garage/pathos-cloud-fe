...
// element with:
//
//     [[pathos:<prefix>/<hash>]]                inline chip
...
// embed: `!` means "show the thing", bare means "point at the thing".
//
...
// [[pathos:posts/ab12…]], [[pathos:posts/ab12…|Read this first]] or
// ![[pathos:nodes/ab12…]] (block embed)
...
export function extractPathosRefs (markdown) {
  ...
  const substituted = masked.replace(PATHOS_REF_RE, (m, bang, prefix, hash, label) => {
...
export function stripPathosRefs (markdown) {
  if (!markdown) return ''
  return markdown.replace(PATHOS_REF_RE, (m, bang, prefix, hash, label) =>
